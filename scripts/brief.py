...
kernel/brief.py --record-verify is completeness, not provenance (case 5b).
...
Other modes are delegated unchanged to kernel/brief.py.
"""
...
def main(argv: list[str]) -> int:
                ...
                json.dumps(
                    {
                        ...
                        "detail": (
                            ...
                            "kernel/brief.py is not provenance"
                        ),
                    },
                    indent=2,
                )
    ...
    if not KERNEL.is_file():
        print("missing kernel/brief.py", file=sys.stderr)
        return 1
